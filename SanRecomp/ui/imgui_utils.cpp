...
void InitImGuiUtils()
{
    // TODO: Replace with europa-grotesk after regenerating font atlas
...
void DrawContainerBox(ImVec2 min, ImVec2 max, float alpha)
{
    ...
    // GTA V Style: Clean dark panel with subtle border - no orange
    auto bgColor = GTA5Style::WithAlpha(GTA5Style::Colors::BackgroundPanel, alpha);
    auto borderColor = GTA5Style::WithAlpha(GTA5Style::Colors::Border, 0.4f * alpha);
    
    // Draw filled background
    drawList->AddRectFilled(min, max, bgColor);
    
    // Draw subtle gray border (thin line)
