...
#include <res/images/installer/gta5_logo.dds.h>
...
// GTA V character images - proper aspect ratio (taller than wide)
constexpr float IMAGE_WIDTH = 600.0f;
constexpr float IMAGE_HEIGHT = 800.0f;

// GTA V Style: Container positioned far right, clear of character
...
static std::unique_ptr<GuestTexture> g_upGTA5Logo;
...
// GTA V Style: Character images at absolute left edge
...
static int g_dlcSelectionIndex = 1; // 0=TBOGT, 1=GTA V, 2=TLAD (center selected by default)
...
// GTA V DLC names for selection screen
...
static void DrawBackground()
{
    ...
    // GTA V Style: Solid black background
    drawList->AddRectFilled({ 0.0, 0.0 }, res, GTA5Style::Colors::Background);
}

static void DrawGTA5Logo()
{
    if (!g_upGTA5Logo)
        return;
    ...
    // Draw GTA V logo in top-right corner - smaller size
    ...
    drawList->AddImage(g_upGTA5Logo.get(), logoMin, logoMax, { 0, 0 }, { 1, 1 }, logoColor);
}

// GTA V Style simple header - replaces common_menu Sonic textures
static void DrawGTA5Header()
{
    auto &res = ImGui::GetIO().DisplaySize;
    auto drawList = ImGui::GetBackgroundDrawList();
    ...
    drawList->AddRectFilled({ 0, stripY }, { res.x, stripY + stripHeight }, GTA5Style::WithAlpha(GTA5Style::Colors::Orange, alpha));
...
static void DrawButtonContainer(ImVec2 min, ImVec2 max, int baser, int baseg, float alpha)
{
    ...
    // GTA V Style: Simple dark row with subtle highlight on hover - no borders
    bool isHovered = (baser != 0 || baseg != 0);
    
    // Simple dark background row
    ImU32 bgColor = isHovered 
        ? GTA5Style::WithAlpha(IM_COL32(50, 50, 50, 255), alpha)
        : GTA5Style::WithAlpha(IM_COL32(30, 30, 30, 200), alpha);
...
static void DrawButton(ImVec2 min, ImVec2 max, const char *buttonText, bool sourceButton, bool buttonEnabled, bool &buttonPressed, float maxTextWidth = FLT_MAX, bool makeDefault = false)
{
    ...
    // GTA V Style: Orange text for selected, white/gray for normal
    ImU32 textColor = isHovered 
        ? GTA5Style::WithAlpha(GTA5Style::Colors::Orange, alpha)
...
static void DrawProgressBar(float progressRatio)
{
    ...
    // GTA V Style: Orange progress bar
    const uint32_t innerColor0 = GTA5Style::Colors::ProgressBg;
    const uint32_t innerColor1 = GTA5Style::Colors::BackgroundDark;
    ...
    drawList->AddRectFilled(min, max, GTA5Style::Colors::BackgroundPanel);
    drawList->AddRect(min, max, GTA5Style::Colors::Border);
    ...
    // GTA V orange progress fill
    const uint32_t sliderColor0 = GTA5Style::Colors::ProgressFill;
    const uint32_t sliderColor1 = GTA5Style::Colors::ProgressFillDark;
    xPadding += Scale(1.5f);
    yPadding += Scale(1.5f);
...
// GTA V Style DLC Selection - Three column layout with character images and logos
static void DrawGTA5DLCSelection()
{
    auto &res = ImGui::GetIO().DisplaySize;
    auto drawList = ImGui::GetBackgroundDrawList();
    float alpha = ComputeMotionInstaller(g_appearTime, g_disappearTime, CONTAINER_INNER_TIME, CONTAINER_INNER_DURATION);
    
    // Calculate column positions (3 columns: TBOGT | GTA V | TLAD)
    ...
    // DLC indices: 0=TBOGT, 1=GTA V (skip), 2=TLAD
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        // TBOGT uses install_002, GTA V uses install_001, TLAD uses install_003
        ...
        // Draw logo image for all columns (DLC logos or GTA V logo)
        ...
        else if (i == 1 && g_upGTA5Logo) logoTexture = g_upGTA5Logo.get();   // GTA V (center)
        else if (i == 2 && g_upTLADLogo) logoTexture = g_upTLADLogo.get();   // TLAD
        
        if (logoTexture)
        {
            // Adjust aspect ratio for GTA V logo (it's wider)
            ...
            if (isSelected)
            {
                ...
                drawList->AddRectFilled(lineMin, lineMax, GTA5Style::WithAlpha(GTA5Style::Colors::Orange, alpha));
            }
            ...
            if (isSelected && isDLC)
            {
                ...
                ImU32 buttonColor = dlcAdded ? GTA5Style::Colors::TextGreen : GTA5Style::Colors::TextOrange;
                drawList->AddText(font, buttonFontSize, buttonPos, GTA5Style::WithAlpha(buttonColor, alpha), buttonText);
            }
        }
    }
    ...
    if (g_escHeld)
    {
        ...
        drawList->AddRectFilled(barMin, barMax, GTA5Style::WithAlpha(IM_COL32(60, 60, 60, 255), alpha));
        drawList->AddRectFilled(barMin, { barMin.x + barWidth * progress, barMax.y }, GTA5Style::WithAlpha(GTA5Style::Colors::Orange, alpha));
        
        const char* skipText = "HOLD ESC TO SKIP...";
        drawList->AddText(hintFont, hintFontSize, { Scale(40), hintY }, GTA5Style::WithAlpha(GTA5Style::Colors::TextOrange, alpha), skipText);
    }
    else
    {
        const char* skipText = "[HOLD ESC] SKIP DLC";
        drawList->AddText(hintFont, hintFontSize, { Scale(40), hintY }, GTA5Style::WithAlpha(GTA5Style::Colors::TextGray, alpha), skipText);
    }
    
    // Right side: navigation hints
    float hintX = res.x - Scale(200);
    const char* hintText = "[CLICK] SELECT";
    drawList->AddText(hintFont, hintFontSize, { hintX, hintY }, GTA5Style::WithAlpha(GTA5Style::Colors::TextGray, alpha), hintText);
}
...
static void DrawTitleUpdateSelection()
{
    ...
    if (updates.empty())
    {
        ...
        drawList->AddText(g_pFntRodin, hintFontSize, { buttonX, hintY }, 
            GTA5Style::WithAlpha(GTA5Style::Colors::TextGray, alpha), hintText);
    }
}
...
static void DrawSources()
{
    ...
    if (g_currentPage == WizardPage::SelectDLC)
    {
        // Use new GTA V style DLC selection
        DrawGTA5DLCSelection();
    }
}
...
static void DrawNavigationButton()
{
    ...
    if (buttonPressed)
    {
        ...
        else if (g_currentPage == WizardPage::SelectGame)
        {
            ...
            {
                // GTA V: Go to Title Update selection after game selection
...
static void CheckCancelAction()
{
    ...
    {
        // Just go back to the previous page, but skip SelectDLC (GTA V has no DLC to install)
...
void InstallerWizard::Init()
{
    ...
    g_upGTA5Logo = LOAD_ZSTD_TEXTURE(g_gta5_logo);
...
void InstallerWizard::Draw()
{
    ...
    // For DLC selection page, use full-screen GTA V style layout
    bool isDLCSelectionPage = (g_currentPage == WizardPage::SelectDLC);
    
    if (!isDLCSelectionPage)
    {
        // GTA V Style: No chevron arrows - skip DrawArrows()
        DrawLeftImage();
        // GTA V Style: Use simple header instead of common_menu with Sonic textures
        DrawGTA5Header();
    ...
    DrawSources(); // This now handles the GTA V DLC selection screen
