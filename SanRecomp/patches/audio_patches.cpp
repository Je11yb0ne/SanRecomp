...
{
    // Update GTA V audio volumes from config
    ...
    if (Config::MusicAttenuation && CanAttenuate())
    {
        ...
        if (os::media::IsExternalMediaPlaying())
        {
            ...
            GTA5Audio::SetMusicVolume(std::lerp(GTA5Audio::GetMusicVolume(), 0.0f, time));
        }
        else
        {
            // Fade music back in
            GTA5Audio::SetMusicVolume(std::lerp(GTA5Audio::GetMusicVolume(), musicVolume, time));
        }
    }
    else
    {
        GTA5Audio::SetMusicVolume(musicVolume);
    }

    GTA5Audio::SetEffectsVolume(effectsVolume);
    GTA5Audio::SetMasterVolume(Config::MasterVolume);
}

// =============================================================================
// GTA V Audio System Stubs and Hooks
// =============================================================================

// GTA V uses RAGE's audEngine for audio management.
// These hooks intercept audio-related calls to provide proper volume control.

// Hook for audio engine initialization
// Address TBD - needs to be found in GTA V code
void GTA5_AudioEngineInit(PPCRegister& result)
{
    GTA5Audio::SetInitialized(true);
    LOGN("GTA5 Audio: Engine initialized");
    result.u32 = 1;  // Success
}

// Hook for setting music volume
// This would be called by the game's options menu
void GTA5_SetMusicVolume(PPCRegister& volume)
{
    float vol = volume.f32;
    GTA5Audio::SetMusicVolume(vol);
    Config::MusicVolume = vol;
}

// Hook for setting effects volume
void GTA5_SetEffectsVolume(PPCRegister& volume)
{
    float vol = volume.f32;
    GTA5Audio::SetEffectsVolume(vol);
    Config::EffectsVolume = vol;
}

// Hook for getting current music volume (for UI display)
void GTA5_GetMusicVolume(PPCRegister& result)
{
    result.f32 = GTA5Audio::GetMusicVolume();
}

// Hook for getting current effects volume (for UI display)
void GTA5_GetEffectsVolume(PPCRegister& result)
{
    result.f32 = GTA5Audio::GetEffectsVolume();
}

// =============================================================================
// XMA Audio Playback Hooks
// =============================================================================
// GTA V uses XMA compressed audio for music and sound effects.
// The XMA decoder is already implemented in apu/xma_decoder.cpp

// Stub for audio stream creation - returns success
// TODO: Find actual GTA V address for audio stream creation
uint32_t GTA5_CreateAudioStream(PPCContext& ctx, uint8_t* base)
{
...
// =============================================================================
// Radio System (GTA V specific)
// =============================================================================
// GTA V has an in-game radio system with multiple stations.
...
namespace GTA5Radio
{
    ...
    float GetRadioVolume() { return g_radioVolume * GTA5Audio::GetMusicVolume(); }
...
void GTA5_SetRadioStation(PPCRegister& station)
{
    int stationId = station.s32;
    GTA5Radio::SetCurrentStation(stationId);
    // Station change logged - LOGN doesn't support format args
}

// Hook for radio volume
void GTA5_SetRadioVolume(PPCRegister& volume)
{
    float vol = volume.f32;
    GTA5Radio::SetRadioVolume(vol);
}
...
void AudioConfigChanged()
{
    ...
    GTA5Audio::SetMasterVolume(Config::MasterVolume);
    GTA5Audio::SetMusicVolume(Config::MusicVolume);
    GTA5Audio::SetEffectsVolume(Config::EffectsVolume);
}

// =============================================================================
// GTA V Audio System PPC Hooks
// =============================================================================
...
PPC_FUNC(sub_822EEDB8) {
    ...
    GTA5Audio::SetInitialized(true);
    LOG_INFO("[Audio] GTA V audio system initialized");
}
...
PPC_FUNC(sub_821AB5F8) {
    ...
    LOG_INFO("[Audio] GTA V radio system initialized");
}
