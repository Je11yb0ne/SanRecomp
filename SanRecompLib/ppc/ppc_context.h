...
#ifndef PPC_LOAD_U32
#if defined(PPC_MMIO_DEBUG)
// Detect repeated reads from the same address (busy-wait on any address).
// After 200+ consecutive reads from a low address (null page),
// force-initialize the address to break linked-list traversal loops.
static inline uint32_t _ppc_busywait_load32(uint8_t* base, uint32_t addr) {
    static uint32_t s_counter = 0;
    static uint32_t s_last_addr = 0;
    static bool s_break_next = false;
    if (addr == s_last_addr) {
        s_counter++;
        if (s_counter == 100) {
        ...
        if (s_counter >= 100) {
            printf("[BUSY-WAIT] Ended at addr=0x%08X after %u reads\n", s_last_addr, s_counter);
            fflush(stdout);
        }
        s_last_addr = addr;
        s_counter = 0;
...
#define PPC_LOAD_U32(x) _ppc_busywait_load32(base, (x))
#else
...
// Guard against corrupted function pointers (game writes to fn table).
#define PPC_CALL_INDIRECT_FUNC(x) \
    ((PPC_LOOKUP_FUNC(base, x) && (uintptr_t)PPC_LOOKUP_FUNC(base, x) != 0xFFFFFFFFFFFFFFFFull) \
