...
bool Runtime::SetupVfs() {
  ...
  // ----- GTA V RPF7 archive mounts (active only when the archives are present) -----
  // GTA V (360) reads its assets as a loose tree under game:\xbox360\ (the merged
  // contents of xbox360a.rpf + xbox360b.rpf), and mounts common.rpf itself via
  // common:\common.rpf. Mount the xbox360 archives as a virtual RPF device so the
  // loose paths resolve; the AES-256 key is read from <game_root>/(360)key.dat
  // (copyrighted, not shipped — absent => RPF mounting is skipped).
  {
    std::vector<std::filesystem::path> xbox360_rpfs;
    for (const char* nm : {"xbox360a.rpf", "xbox360b.rpf"}) {
      auto p = abs_game_root / nm;
      if (std::filesystem::exists(p)) xbox360_rpfs.push_back(p);
    }
    // The 8GB install: partN.rpf RPF7 archives (extracted from the STFS content
    // packages to <game_root>\install\) hold the bulk the disc archives lack
    // (levels/models/anim/...). RpfContainerDevice merges them all into one tree.
    for (const char* nm : {"part0.rpf", "part1.rpf", "part2.rpf", "part3.rpf"}) {
      auto p = abs_game_root / "install" / nm;
      if (std::filesystem::exists(p)) xbox360_rpfs.push_back(p);
    }
    auto key_path = abs_game_root / "(360)key.dat";
    if (!xbox360_rpfs.empty() && std::filesystem::exists(key_path)) {
      uint8_t key[32] = {};
      bool key_ok = false;
      if (FILE* kf = rex::filesystem::OpenFile(key_path, "rb")) {
        key_ok = fread(key, 1, 32, kf) == 32;
        fclose(kf);
      }
      if (key_ok) {
        const char* rpf_mount = "\\Device\\RpfXbox360";
        auto rpf_device =
            std::make_unique<rex::filesystem::RpfContainerDevice>(rpf_mount, xbox360_rpfs, key);
        if (rpf_device->Initialize() && file_system_->RegisterDevice(std::move(rpf_device))) {
          // game:\xbox360\<dir>\... resolves game: -> Partition1, then this
          // device-path symlink -> the RPF device (ResolveSymbolicLink loops
          // until stable). The trailing separator is REQUIRED so the symlink
          // matches only the xbox360\ directory, NOT the sibling archive files
          // xbox360a.rpf / xbox360b.rpf (which the game opens + mounts itself).
          file_system_->RegisterSymbolicLink("\\Device\\Harddisk0\\Partition1\\xbox360\\",
                                              "\\Device\\RpfXbox360\\");
          REXSYS_INFO("  Mounted GTA V xbox360 RPF archives at game:\\xbox360\\");
        } else {
          REXSYS_ERROR("  Failed to initialize GTA V xbox360 RPF device");
        }
      } else {
        REXSYS_WARN("  GTA V RPF key unreadable: {}", key_path.string());
      }
    }
    // common.rpf is mounted by the game itself; make common:\ resolve to the game
    // root so common:\common.rpf finds the real archive file.
    if (std::filesystem::exists(abs_game_root / "common.rpf")) {
      file_system_->RegisterSymbolicLink("common:", mount_path);
      REXSYS_DEBUG("  Registered symbolic link: common: -> game root");
    }
  }
