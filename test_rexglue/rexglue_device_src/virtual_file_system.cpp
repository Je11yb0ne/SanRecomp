...
Entry* VirtualFileSystem::ResolvePath(const std::string_view path) {
  ...
  // Find the longest mount path that matches, then OVERLAY all devices sharing
  // that longest mount path (union semantics): try each until one resolves the
  // entry. This lets several archives merge at a single mount point (e.g.
  // game:\xbox360\ served by the disc RPF device + the install packages) while
  // preserving longest-prefix selection so a broad device (e.g. NullDevice at
  // \Device\Harddisk0) never shadows a more specific one (\...\Partition1).
  size_t best_len = 0;
  bool any_match = false;
  for (const auto& d : devices_) {
    if (rex::string::utf8_starts_with_case(normalized_path, d->mount_path())) {
      any_match = true;
      if (d->mount_path().size() > best_len) best_len = d->mount_path().size();
    }
  }
  if (!any_match) {
  ...
  const Device* last_device = nullptr;
  for (const auto& d : devices_) {
    if (d->mount_path().size() != best_len ||
        !rex::string::utf8_starts_with_case(normalized_path, d->mount_path())) {
      continue;
    }
    last_device = d.get();
    auto relative_path = normalized_path.substr(d->mount_path().size());
    auto* entry = d->ResolvePath(relative_path);
    if (entry) {
      if (had_symlink) {
        REXFS_TRACE("VFS resolved '{}' via symlink '{}' on device '{}' -> '{}'", path,
                    normalized_path, d->mount_path(), entry->absolute_path());
      } else {
        REXFS_TRACE("VFS resolved '{}' on device '{}' -> '{}'", path, d->mount_path(),
                    entry->absolute_path());
      }
      return entry;
    }
  ...
  if (had_symlink) {
    REXFS_WARN("VFS: entry not found for '{}' (via symlink '{}') on device '{}'", path,
               normalized_path, last_device ? last_device->mount_path() : "");
  } else {
    REXFS_WARN("VFS: entry not found for '{}' on device '{}'", path,
               last_device ? last_device->mount_path() : "");
  }
  return nullptr;
}
